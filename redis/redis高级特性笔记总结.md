...
> AOF方式有一个问题就是，redis的会将写入操作保存到日志文件，那么AOF的日志文件是不是变得特别大？redis可用内存空间满了还会根据淘太策略（）删除键值，删除的键还需要保存吗？。这里涉及到AOF日志文件膨胀到一定大小时会有**rewrite操作**。基于当时redis内存中的数据构造一个更小、更精简的AOF文件，然后将旧的AOF文件给删除了。
