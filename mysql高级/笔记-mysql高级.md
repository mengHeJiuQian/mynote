...
2. 行锁：一次性对一条记录加锁。开销大，加锁慢，可能会出现死锁。
	...
	2. 如果不同会话操作不同数据，无影响。
	3. 间隙锁：行锁的一种特殊情况，值在范围内，但却不存在（例如，主键为自增id，更新语句的where条件是id>0 且id<10，但id=6的记录被删除而不存在，所以执行更改时，锁的区间可能是id范围内的记录，即使id=6的这条数据不存在，id=7也像被加锁一样被称为间隙锁，别的会话不能更改id=6）。
...
2. slave的IO线程将master的bin-log拷贝到自己的relay log
![mysql主从复制原理图.png](0)
